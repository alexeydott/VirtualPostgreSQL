    ...
    VpsError *error)
{
    static const char bootstrap[] =
        ...
        "DROP TABLE IF EXISTS public.vps_stage11_composite; "
        "DROP TABLE IF EXISTS public.vps_stage11_dml; "
        "CREATE TABLE public.vps_stage11_dml ("
        "id pg_catalog.int8 GENERATED ALWAYS AS IDENTITY PRIMARY KEY,"
        "payload pg_catalog.text NOT NULL DEFAULT 'defaulted',"
        "nullable pg_catalog.text,bytes pg_catalog.bytea,"
        "amount pg_catalog.numeric(20,4) NOT NULL DEFAULT 0,"
        "version pg_catalog.int8 NOT NULL DEFAULT 1,"
        "generated pg_catalog.text GENERATED ALWAYS AS "
        "(payload || ':' || version::pg_catalog.text) STORED); "
        "CREATE INDEX vps_stage11_dml_payload_idx ON public.vps_stage11_dml(payload); "
        "CREATE TABLE public.vps_stage11_composite ("
        "tenant pg_catalog.text NOT NULL,code pg_catalog.int4 NOT NULL,"
        "payload pg_catalog.text,PRIMARY KEY(tenant,code)); "
        "END $vps$";
