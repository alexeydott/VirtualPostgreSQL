...
foreach ($name in $required) {
    if ([string]::IsNullOrEmpty([Environment]::GetEnvironmentVariable($name))) {
        throw "[stage11-stand] required runtime environment variable is missing: $name"
    }
}
...
foreach ($name in $temporary) { $saved[$name] = [Environment]::GetEnvironmentVariable($name) }
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw '[stage11-stand] fixture bootstrap failed' }
    ...
    $env:VPS_VTAB_TEST_DML = '1'
    & pwsh -NoProfile -File (Join-Path $PSScriptRoot 'test-vtable-stand.ps1') `
        -Root $Root -BuildDirectory $BuildDirectory
    if ($LASTEXITCODE -ne 0) { throw '[stage11-stand] DML contour failed' }
} finally {
    foreach ($name in $temporary) {
        [Environment]::SetEnvironmentVariable($name, $saved[$name])
    }
}
Write-Output 'stage11_stand status=passed operations=insert,update,delete optimistic=column,xmin identity=integer,composite'
