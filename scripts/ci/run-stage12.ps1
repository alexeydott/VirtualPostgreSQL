...
$gate = 'stage12-transactions-savepoints'
...
if ($RunLocalStand) {
    & pwsh -NoProfile -File (Join-Path $PSScriptRoot 'test-stage12-stand.ps1') `
        -Root $rootPath
    if ($LASTEXITCODE -ne 0) { throw 'local Stage 12 stand failed' }
}
