...
if ($env:OS -ne 'Windows_NT') { throw '[stage7-windows] Windows host required' }
$before = @(& git -C $rootPath status --porcelain=v1 --untracked-files=all)
if ($LASTEXITCODE -ne 0) { throw '[stage7-windows] unable to read source-tree state' }
$started = [Diagnostics.Stopwatch]::StartNew()

& pwsh -NoProfile -File (Join-Path $PSScriptRoot 'test-source-tree.ps1') -Root $rootPath
if ($LASTEXITCODE -ne 0) { throw '[stage7-windows] source-tree gate failed' }
...
foreach ($preset in @('msvc-x86-debug', 'msvc-x64-release', 'clang-cl-x64-debug', 'clang-cl-x64-asan')) {
    ...
    if ($LASTEXITCODE -ne 0) { throw "[stage7-windows] build preset failed: $preset" }
}
...
if ($RunLocalStand) {
    ...
    if ($LASTEXITCODE -ne 0) { throw '[stage7-windows] local stand failed' }
}

$after = @(& git -C $rootPath status --porcelain=v1 --untracked-files=all)
if ($LASTEXITCODE -ne 0) { throw '[stage7-windows] unable to re-read source-tree state' }
