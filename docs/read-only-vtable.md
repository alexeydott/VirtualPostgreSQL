...
Currently, `VirtualPostgreSQL` exposes an end-to-end read path. `xCreate` and
`xConnect` parse strict module arguments, resolve credentials, connect
immediately, and prepare/describe a bounded table or query `SELECT`. A literal
`connstr` in a persistent virtual-table declaration remains visible in
`sqlite_schema`; use `credential_ref` or `service` for production secrets.
SQLite schema declaration is derived from the server result descriptor; a
failed connection or descriptor never falls back to a dummy schema.

`xFilter` leases an exclusive async client connection, creating one when the
pool has no reusable match, and starts an extended
...
pushdown are described in the planner guide.
...
Other explicit keys expose a format-tagged, length-prefixed hidden identity token.
...
The current Windows gate builds MSVC Win32 Debug, MSVC x64 Release, clang-cl x64
...
The streaming/concurrency contour adds explicit host cancellation, checked scan
